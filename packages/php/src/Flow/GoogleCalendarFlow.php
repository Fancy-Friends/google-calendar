 ...
 */
/**
 * Every Google Calendar node executor — the PHP twin of
 * `GOOGLE_CALENDAR_RUNNABLE_KINDS` in @particle-academy/google-calendar-js.
 *
 * Each class declares its kind in a `#[FlowNode]` attribute. Two ways onto a
 * host:
 *
 * - `GoogleCalendarFlow::register($kinds, $executors)` — one call, no
 * directory scan.
 ...
 */
final class GoogleCalendarFlow
{
    /** @var list<class-string> */
    public const EXECUTORS = [
        ChannelStopExecutor::class,
        ChannelWatchExecutor::class,
        EventGetExecutor::class,
        EventListExecutor::class,
        EventsChangedTriggerExecutor::class,
    ];
