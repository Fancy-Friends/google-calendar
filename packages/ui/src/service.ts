...
/**
 * Google Calendar's identity on the authoring surface, shared by every Google
 * Calendar node.
 *
...
/** The credentials a Google Calendar connection holds. */
export const GOOGLE_CALENDAR_CREDENTIALS = [
  ...
  },
  {
    ...
    "scope": "account",
    "secret": true,
    "help": "A secret the host chooses per connected account and sends as `token` on every events.watch. Google echoes it back in the X-Goog-Channel-Token header of every notification, and that echo is the ONLY thing that authenticates a delivery -- Google does not sign them. Up to 256 characters."
  }
] as const;

/**
 * The OAuth2 exchange Google Calendar requires — DECLARED here, performed by
 * the host.
 *
...
/** Build a Google Calendar node's connector metadata from the operation it performs. */
export function googleCalendarMeta(
  role: ConnectorMeta["role"],
  operation: string,
  docs: string,
): ConnectorMeta {
  return { ...GOOGLE_CALENDAR_SERVICE, role, operation, docs };
}
