...
import { GOOGLE_CALENDAR_KINDS } from "../src/index.js";
...
test("the registry names are exactly what the manifest says", () => {
  assert.deepEqual(GOOGLE_CALENDAR_KINDS.map((kind) => kind.name), [
    "@particle-academy/google_calendar_channel_stop",
    "@particle-academy/google_calendar_channel_watch",
    "@particle-academy/google_calendar_event_get",
    "@particle-academy/google_calendar_event_list",
...
test("a trigger takes no input port — it starts a run rather than continuing one", () => {
  assert.deepEqual(GOOGLE_CALENDAR_KINDS.find((k) => k.name.endsWith("_events_changed_trigger"))?.inputs, []);
});
